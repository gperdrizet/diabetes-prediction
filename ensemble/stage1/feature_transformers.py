...
def get_transformer(transformer_name: str, **hyperparameters):
    """Get a transformer instance by name with given hyperparameters.
    
    Factory function that maps transformer names to their classes and
    instantiates them with the provided hyperparameters.
    
    Args:
        transformer_name: Name of the transformer (e.g., 'ratio', 'product')
        **hyperparameters: Hyperparameters to pass to the transformer
    ...
    Example:
        >>> transformer = get_transformer('ratio', n_features=10, random_state=42)
        >>> transformer = get_transformer('kde', bandwidth='scott')
    """
    transformer_map = {
        'clean_numeric': CleanNumericTransformer,
        'random_feature_selector': RandomFeatureSelector,
    ...
    if transformer_name not in transformer_map:
        available = ', '.join(sorted(transformer_map.keys()))
        raise ValueError(
            f"Unknown transformer '{transformer_name}'. "
            f"Available transformers: {available}"
        )
    
    transformer_class = transformer_map[transformer_name]
    return transformer_class(**hyperparameters)
...
def get_available_transformers():
    ...
    Returns:
        List of transformer names that can be used with get_transformer()
    """
    return [
        'clean_numeric', 'random_feature_selector',
        'ratio', 'product', 'difference', 'sum',
        'reciprocal', 'square', 'sqrt', 'log',
        'binning', 'kde', 'kmeans',
        'noise_injector'
    ]
